...
function Terminal(file, args, opts) {
  ...
  var env, cwd, name, size, cols, rows, term,
      self = this,
      defaultCols = 80,
      defaultRows = 24;
  ...
  if (opts.snap === true && process.stdout.isTTY === true) {
    ...
    if (isValidPositiveInteger(process.stdout.columns)) {
      defaultCols = process.stdout.columns;
    }
    if (isValidPositiveInteger(process.stdout.rows)) {
      defaultRows = process.stdout.rows;
    }
  ...
  cols = isValidPositiveInteger(opts.cols) ? opts.cols : defaultCols;
  rows = isValidPositiveInteger(opts.rows) ? opts.rows : defaultRows;

  if (opts.snap === true && process.stdout.isTTY === true && cols === defaultCols && rows === defaultRows) {

    // Delay for 1 cycle just to ensure that the "resize" event doesn't trigger
    // when the terminal is first opened
    ...
    // Curry all data received straight to stdout
...
Terminal.open = function(opts) {
  ...
  var cols = isValidPositiveInteger(opts.cols) ? opts.cols : self._defaults.cols
    , rows = isValidPositiveInteger(opts.rows) ? opts.rows : self._defaults.rows
    , term;
...
Terminal.prototype.resize = function(cols, rows) {
  cols = isValidPositiveInteger(cols) ? cols : this.cols;
  rows = isValidPositiveInteger(rows) ? rows : this.rows;
